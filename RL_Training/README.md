...
python3 train.py \
  --data-dirs \
    ../Omnet_Sims/dc_simulations/simulations/sims/tmp/data/data_1G_dibs \
    ../Omnet_Sims/dc_simulations/simulations/sims/tmp/data/data_1G_ecmp \
    ../Omnet_Sims/dc_simulations/simulations/sims/tmp/data/data_1G_probabilistic \
  --algo iql --out-dir runs/iql_baseline --steps 200000 --batch-size 2048

# OR: parametric discovery via base+policies
python3 train.py \
  --data-base ../Omnet_Sims/dc_simulations/simulations/sims/tmp/data \
  --policies dibs ecmp probabilistic probabilistic_tb random random_tb sd threshold threshold_tb uniform_random vertigo \
...
3) Evaluate with FQE (run automatically every few K steps; can also run standalone):

```
python3 train.py --data-base ../Omnet_Sims/dc_simulations/simulations/sims/tmp/data \
  --policies dibs ecmp probabilistic ... \
  --algo iql --out-dir runs/iql_eval_only --steps 0 --fqe-eval
```
...
- Instant: `queue_util`, `queues_tot_util`, `seq_num_norm`, `flow_age_norm`, `packet_latency_norm`, `is_incast`, `query_age_norm`, `ooo_recent`, `deflect_ema`.
...
Reward per step:
```
r = - (w_q*queue_util + w_l*packet_latency_norm + w_o*ooo) - w_d*[DEFLECT]
```
...
Weights (tunable via CLI): `w_q=1.0, w_l=0.3, w_o=0.2, w_d=0.05, w_F=0.5`.
...
- `fqe_eval.json`: FQE returns and confidence intervals when enabled.
