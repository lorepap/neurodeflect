...
class RLBaselineComparator:
    ...
    def load_threshold_baselines(self):
        ...
        # Find threshold dataset files in the simulations directory
        sims_dir = Path('/home/ubuntu/practical_deflection/Omnet_Sims/dc_simulations/simulations/sims')
        threshold_files = list(sims_dir.glob('threshold_dataset_*.csv'))
        
        if not threshold_files:
            print("  No threshold baseline datasets found")
            print(f"  Looked in: {sims_dir}")
            return False
